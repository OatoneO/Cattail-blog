/**
 * 博客知识图谱处理服务
 * 负责将博客内容处理为知识图谱三元组数据，提取实体节点和实体间关系
 */
...
function extractEntityToEntityRelationships(blog, nodes, entities, text) {
  ...
  const existingEntityIds = nodes
    .filter(node => node.type === 'noun_entity')
    .map(node => node.id);
...
/**
 * 检测实体间的关系类型
 * @param {string} source 源实体文本
 * @param {string} target 目标实体文本
 * @param {string} text 文档文本
 * @returns {string|null} 关系类型或null
 */
